...
export function MailList({ emails, isLoading, isFetching, onDelete, isDeleting, onRefresh, selectedIds, setSelectedIds, isAddressCreated, onSelectEmail, showViewPasswordButton, onShowPassword, selectedEmail, onCloseDetail }: MailListProps) {
  ...
  return (
    <div className="rounded-md border border-cyan-50/20 text-white flex flex-col">
      {/* 邮件列表头部 */}
      ...
      {/* 邮件列表主体 */}
      {/* fix: 无论是否显示详情，都保持 grids 背景和最小高度，以确保一致性。在列表视图中使用固定高度，在详情视图中使用最小高度以允许内容扩展。 */}
      <div className={clsx("flex flex-col overflow-y-auto p-2 grids", !selectedEmail ? "h-[488px]" : "min-h-[488px]")}>
        {renderBody()}
      </div>
    </div>
  );
